...
Canvas3dManager::Canvas3dManager()
{
    ...
    vecColorWhite = Vector4(1, 1, 1, 1);
    elPosition.set("inPos", VERTEX_TYPE_POSITION, 0, VERTEX_FORMAT_FLOAT4);
    elNormal.set("inNormal", VERTEX_TYPE_NORMAL, 0, VERTEX_FORMAT_FLOAT4);
    elColor.set("inColor", VERTEX_TYPE_COLOR, 0, VERTEX_FORMAT_FLOAT4);
    elColorUint.set("inColor", VERTEX_TYPE_COLOR, 0, VERTEX_FORMAT_COLOR);
    elUV.set("inUV", VERTEX_TYPE_TEXCOORD, 0, VERTEX_FORMAT_FLOAT2);
    declPos3D.set(&elPosition, 0);
    declPos3DNormal.set(&elPosition, &elNormal, 0);
    declPosUV3D.set(&elPosition, &elUV, 0);
}
...
int Canvas3dManager::stop()
{
    ...
    {
        Canvas3dThread& currentThread = thread[threadId];
        if(currentThread.pBatchMesh)
        {
            currentThread.pBatchMesh->freeBuffer();
        }
    }
    return 0;
}
