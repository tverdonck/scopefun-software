...
Canvas2dManager::Canvas2dManager()
{
    ...
    CBatchSize = 10000;
    elPosition.set("inPos", VERTEX_TYPE_POSITION, 0, VERTEX_FORMAT_FLOAT2);
    elUV.set("inUV", VERTEX_TYPE_TEXCOORD, 0, VERTEX_FORMAT_FLOAT2);
    declPos2D.set(&elPosition, 0);
    declPosTex2D.set(&elPosition, &elUV, 0);
}
...
void Canvas2dManager::endBatch(uint threadId, int color, GrTexture* pTexture, BlendMode bm, SamplerState ss, GrShader* shader)
{
    ...
    else
    {
        if(pTexture)
        {
            currentSlot.pShader = textureShader;
        }
        else
        {
            currentSlot.pShader = normalShader;
        }
    }
