...
int String::pos(const String& substr, int start) const
{
    ...
    if(start + substr.getLength() > length)
    {
        return -1;
    }
    for(int a = start; a < length - substr.getLength() + 1; a++)
    {
        if(memcmp(&data[a], (void*)substr.data, substr.getLength()) == 0)
        {
            return a;
        }
    }
...
int String::posReverse(const String& substr, int end) const
{
    ...
    if(end + substr.getLength() > length)
    {
        return -1;
    }
...
void String::remove(int pos, int cnt)
{
    ...
    if(pos + cnt > length)
    {
        CORE_ABORT("pos + cnt out of range", 0);
    }
...
void String::replace(const String thisstr, const String withthis, int start)
{
    ...
    {
        CORE_ABORT("start out of range", 0);
    }
    while(start < length)
    {
...
{
    sprintf(&data[pos], "%s", str);
    pos += strlen(str);
}

int  StringBuffer::operator << (const char* str)
{
    add(str);
    return 0;
}

int  StringBuffer::operator << (int integer)
{
    char buffer[1024] = {0};
...
int  StringBuffer::operator << (float floating)
{
    char buffer[1024] = {0};
...
int  StringBuffer::operator << (double number)
{
    char buffer[1024] = {0};
