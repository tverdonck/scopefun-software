...
void OsciloscopeThreadRenderer::renderAnalogGrid(uint threadId, OsciloscopeThreadData& threadData)
{
    ...
    float z = 0.f;
    {
        ...
        // half point offset
        double halfPointOffset = 1.0 / (double)(threadData.m_frame.capture);
              halfPointOffset /= 2.0;
...
void OsciloscopeThreadRenderer::renderAnalogAxis(uint threadId, OsciloscopeThreadData& threadData)
{
    ...
    pCanvas3d->bLine(threadId, Vector4(sx + xMin, 0, 0, 1), Vector4(sx + xMax, 0, 0, 1));
    pCanvas3d->endBatch(threadId, render.cameraOsc.Final, COLOR_ARGB(255, 255, 0, 0));
    ////////////////////////////////////////////////////////////////////////////////
    // y axis
    ////////////////////////////////////////////////////////////////////////////////
    pCanvas3d->beginBatch(threadId, CANVAS3D_BATCH_LINE, 1);
    pCanvas3d->bLine(threadId, Vector4(sx + xMin, yMin, 0, 1), Vector4(sx + xMin, yMax, 0, 1));
...
void OsciloscopeThreadRenderer::renderAnalogUnits(uint threadid, OsciloscopeThreadData& threadData)
{
    ...
    ToolText::Time(bufferTime, 1024, gridTime);
    ...
    pFont->writeText(threadid, 200, 15, formatBuffer, render.colorChannel0);
    FORMAT("Ch1 [ %s / div ]", bufferVolt1);
    pFont->writeText(threadid, 350, 15, formatBuffer, render.colorChannel1);
    ////////////////////////////////////////////////////////////////////////////////
    // size
    ////////////////////////////////////////////////////////////////////////////////
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        if(i == 1 && wndMain.channel02.OscOnOff == 0)
        {
            continue;
        }
        double step = 0.f;
...
void OsciloscopeThreadRenderer::measureSignal(uint threadId, OsciloscopeThreadData& threadData, MeasureData& measure, OsciloscopeFFT& fft)
{
    ...
    time1.popFront();
    timeF.popFront();
    if(time0.getCount() % 2 == 1)
    {
        time0.popBack();
    }
    if(time1.getCount() % 2 == 1)
    {
        time1.popBack();
    }
    if(timeF.getCount() % 2 == 1)
    {
        timeF.popBack();
    }
    double period0 = 0;
    double period1 = 0;
    double periodF = 0;
    for(int i = 0; i < time0.getCount(); i++)
    {
        period0 += time0[i];
    }
    ...
    for(int i = 0; i < timeF.getCount(); i++)
    {
        periodF += timeF[i];
    }
    ...
    // division by zero
    if(current.row[Ch0Tperiod] == 0.0)
    {
        current.row[Ch0Tperiod] = 1.0;
    }
    if(current.row[Ch1Tperiod] == 0.0)
    {
        current.row[Ch1Tperiod] = 1.0;
    }
    if(current.row[FunTperiod] == 0.0)
    {
        current.row[FunTperiod] = 1.0;
    }
...
void OsciloscopeThreadRenderer::renderAnalog(uint threadId, OsciloscopeThreadData& threadData, float z, uint channelId, uint shadow, SDisplay& frame, float captureTime, float captureVolt, uint color, bool invert)
{
   ...
   for (uint point = start + increment; point <= end; point += increment)
   {
    ...
    if(isamples)
    {
        ...
        {
            // horizontal
            double halfPoint = 1.0 / (double)sampleCount;
                  halfPoint *= 0.5;
            for (uint point = start; point <= end; point ++)
            {
               uint idx0 = clamp<uint>(point, start, end);
    ...
    if(wndMain.display.fftUnits)
    {
        ...
        if(wndMain.display.fftLogFreq == 1)
        {
            for(int i = 0; i < 12; i++)
            {
                ...
                if(ypos % 2 == 0)
                {
                    pFont->writeText3d(threadid, render.cameraFFT.Final, x, charHeight - 0.5f, 0.f, Vector4(1, 0, 0, 1), Vector4(0, 1, 0, 1), formatBuffer, 0xffffffff, render.fftScaleX, render.fftScaleY);
                }
                else
                {
                    pFont->writeText3d(threadid, render.cameraFFT.Final, x, -charHeight / 2 - 0.5f, 0.f, Vector4(1, 0, 0, 1), Vector4(0, 1, 0, 1), formatBuffer, 0xffffffff, render.fftScaleX, render.fftScaleY);
                }
                ypos++;
            }
            for(int i = 0; i < 32; i++)
...
void OsciloscopeSlider::MinMax(float& xMinimum, float& xMaximum, uint width, uint height, float viewX, float size, float zoom)
{
    float rectW = 0;
    float rectH = 0;
    float rectX = 0;
