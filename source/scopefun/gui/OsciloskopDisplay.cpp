...
void OsciloskopDisplay::DisplayOnInitDialog(wxInitDialogEvent& event)
{
    ...
    m_textCtrlFFT->SetValue(wxString::FromAscii(pFormat->floatToString(pOsciloscope->window.display.fftWidth)));
...
void OsciloskopDisplay::m_textCtrlSignalOnTextEnter(wxCommandEvent& event)
{
    ...
    pOsciloscope->window.display.signalWidth = pFormat->stringToFloat(m_textCtrlSignal->GetValue().ToAscii().data());
    m_sliderSignal->SetValue((pOsciloscope->window.display.signalWidth / MAX_TRIANGLE_SIZE) * 100);
}
...
void OsciloskopDisplay::m_sliderSignalOnScroll(wxScrollEvent& event)
{
    ...
    m_textCtrlSignal->SetValue(pFormat->floatToString(fvalue));
}
...
void OsciloskopDisplay::m_textCtrlFFTOnTextEnter(wxCommandEvent& event)
{
    ...
    pOsciloscope->window.display.fftWidth = pFormat->stringToFloat(m_textCtrlFFT->GetValue().ToAscii().data());
    m_sliderFFT->SetValue((pOsciloscope->window.display.fftWidth / MAX_TRIANGLE_SIZE) * 100);
}
...
void OsciloskopDisplay::m_sliderFFTOnScroll(wxScrollEvent& event)
{
    ...
    m_textCtrlFFT->SetValue(pFormat->floatToString(fvalue));
}
...
{
    uint tesselation3d = pFormat->stringToInteger(m_textCtrl3dTessalation->GetValue().ToAscii().data());
    ...
    m_textCtrl3dTessalation->SetValue(wxString(pFormat->integerToString(tesselation3d)));
}
...
{
    pOsciloscope->window.display.tessalation2d = pFormat->stringToInteger(m_textCtrl2dTessalation->GetValue().ToAscii().data());
    m_slider2dTessalation->SetValue(pOsciloscope->window.display.tessalation2d);
}
